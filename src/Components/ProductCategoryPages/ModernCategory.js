...
import { getModernProducts,getProducts} from "../../Redux/Actions/ProductAction";
...
class ModernCategory extends Component {
  constructor(props) {
    super(props);
    this.state = {
      modernList: [],
  ...
  handleChange = (searchTerm) => (event) => {
    ...
    if (this.state.searchTerm !== "") {
      const newlistProducts = this.props.modernList.filter((listProduct) => {
      ...
      this.setState({ searchResults: this.props.modernList });
    }
  };
                    ...
                    <div className="aa-product-inner">
                      ...
                      <div className="tab-pane  in active" id="men">
                        {this.state.searchTerm.length < 1
                          ? this.props.modernList.map((listProduct) => (
...
function mapStateToProps(state) {
  return {
    modernList: state.ProductReducer.modernList,
