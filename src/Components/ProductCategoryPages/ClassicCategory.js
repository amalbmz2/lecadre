...
import { getClassicProducts,getProducts} from "../../Redux/Actions/ProductAction";
...
class ClassicCategory extends Component {
  constructor(props) {
    super(props);
    this.state = {
      classicList: [],
  ...
  handleChange = (searchTerm) => (event) => {
    ...
    if (this.state.searchTerm !== "") {
      const newlistProducts = this.props.classicList.filter((listProduct) => {
      ...
      this.setState({ searchResults: this.props.classicList });
    }
  };
                    ...
                    <div className="aa-product-inner">
                      ...
                      <div className="tab-pane  in active" id="men">
                        {this.state.searchTerm.length < 1
                          ? this.props.classicList.map((listProduct) => (
...
function mapStateToProps(state) {
  return {
    classicList: state.ProductReducer.classicList,
